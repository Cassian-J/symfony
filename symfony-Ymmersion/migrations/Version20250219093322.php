 ...
 */
final class Version20250219093322 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__task AS SELECT id, title, description, color, periodicity, UserUuid, GroupUuid, difficulty FROM task');
        ...
        , difficulty INTEGER NOT NULL, CONSTRAINT FK_527EDB252E46E5BF FOREIGN KEY (UserUuid) REFERENCES users (user_uuid) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_527EDB253D458CB8 FOREIGN KEY (GroupUuid) REFERENCES groups (group_uuid) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO task (id, title, description, color, periodicity, UserUuid, GroupUuid, difficulty) SELECT id, title, description, color, periodicity, UserUuid, GroupUuid, difficulty FROM __temp__task');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE INDEX IDX_F11D61A2558828B5 ON invitation (Recever)');
        $this->addSql('CREATE INDEX IDX_F11D61A2A4872AF8 ON invitation (WhichGroup)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__task AS SELECT id, title, description, color, periodicity, difficulty, UserUuid, GroupUuid FROM task');
        $this->addSql('DROP TABLE task');
        $this->addSql('CREATE TABLE task (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, title VARCHAR(250) NOT NULL, description CLOB DEFAULT NULL, color VARCHAR(6) NOT NULL, periodicity VARCHAR(255) NOT NULL, difficulty INTEGER NOT NULL, UserUuid CHAR(36) DEFAULT NULL --(DC2Type:guid)
        , GroupUuid CHAR(36) DEFAULT NULL --(DC2Type:guid)
        ...
        $this->addSql('INSERT INTO task (id, title, description, color, periodicity, difficulty, UserUuid, GroupUuid) SELECT id, title, description, color, periodicity, difficulty, UserUuid, GroupUuid FROM __temp__task');
        $this->addSql('DROP TABLE __temp__task');
        $this->addSql('CREATE INDEX IDX_527EDB252E46E5BF ON task (UserUuid)');
        $this->addSql('CREATE INDEX IDX_527EDB253D458CB8 ON task (GroupUuid)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__users AS SELECT user_uuid, email, pseudo, pwd, last_connection, GroupUuid FROM users');
